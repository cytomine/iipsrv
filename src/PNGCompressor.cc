...
PNGCompressor::CompressStrip( unsigned char *inputbuff, unsigned char *outputbuff,
  ...
  if (((png_bytepp) malloc( tile_height * sizeof( png_bytep ))) == NULL ) {
    throw "PNGCompressor:: Out of memory";
  }

  // ensure that the output buffer is set properly
  setOutputBuffer( outputbuff, bpc/8*width*channels*tile_height+65536 );
...
void PNGCompressor::addXMPMetadata( const string &xmp_metadata ) {
  ...
  ************************************************************/
  ...
  int i = 0;
  memcpy( &chunk[i], xmp_keyword.c_str(), xmp_keyword.size());
  i += xmp_keyword.size();        // keyword
  chunk[i] = '\0';
  i += 1;                         // null string terminated
  chunk[i] = 0;
  i += 1;                         // compression flag set to 0
  chunk[i] = PNG_ITXT_COMPRESSION_NONE;
  i += 1;                         // compression method
  chunk[i] = '\0';
  i += 1;                         // language tag is empty string with null termination
  chunk[i] = '\0';
  i += 1;                         // translated keyword is empty string with null termination
  memcpy( &chunk[i], xmp_metadata.c_str(), xmp_metadata.size());
  i += xmp_metadata.size();
